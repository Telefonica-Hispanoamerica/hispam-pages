...
export default function StylePropertyField({
	...
	let inputToRender = (
    	...
    	case 'composite':
      	{
			...
			inputToRender = (
				<div
					className='flex flex-wrap p-2 bg-black/20'
				>
  	...
  	return (
		<div
			{...rest}
			className={cx('styles-content mb-3 px-1', prop.isFull() ? 'w-full' : 'w-half')}
		>
      		<div className={cx('head-subproperties', canClear && 'text-sky-300')}>
        		<div className="flex-grow capitalize">{prop.getLabel()}</div>
				{canClear && (
					<button onClick={() => prop.clear()} className='class-delete'>
